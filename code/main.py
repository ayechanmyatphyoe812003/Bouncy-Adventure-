...
from database import load_player_progress
# from menu import menu


class Game:
    def __init__(self, player_id, menu_func):
        pygame.init()
        self.display_surface = pygame.display.set_mode((WINDOW_WIDTH, WINDOW_HEIGHT))
        pygame.display.set_caption('Bouncy Adventure')
        self.clock = pygame.time.Clock()
        self.import_assets()
        self.player_id = player_id
        self.menu_func = menu_func  # Store menu function reference


        self.ui = UI(self.font, self.ui_frames)
        self.data = Data(self.ui)
        self.data.unlocked_level = load_player_progress(self.player_id)
        self.tmx_maps = {
    ...
    def switch_stage(self, target, unlock=0):
        if target == 'level':
            self.current_stage = Level(self.tmx_maps[self.data.current_level], self.level_frames, self.audio_files,
                                       self.data, self.switch_stage,self.player_id)

        else:  # overworld
            if unlock > 0:
                self.data.unlocked_level = min(self.data.unlocked_level, 5)  # Unlock next level
            else:
                self.data.health -= 1
            self.current_stage = Overworld(self.tmx_overworld, self.data, self.overworld_frames, self.switch_stage)
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if event.key == pygame.K_ESCAPE:
                        self.menu_func(self.player_id)  # Call menu function with player_id
