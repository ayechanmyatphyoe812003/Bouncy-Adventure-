...
# manages the sprites in the game world, including background and main sprites
class WorldSprites(pygame.sprite.Group):
    def __init__(self, data):
        super().__init__()
        self.display_surface = pygame.display.get_surface()
        self.data = data
        self.offset = vector()  # Initializes the offset for camera movement

    def draw(self, target_pos):
        self.offset.x = -(target_pos[0] - WINDOW_WIDTH / 2)
        self.offset.y = -(target_pos[0] - WINDOW_HEIGHT / 2)
        # Calculates the offset based on the target position

        # background
        # Draws background sprites sorted by their z-layer
        ...
        # main
        # Draws main sprites sorted by their vertical position (rect.centery)
