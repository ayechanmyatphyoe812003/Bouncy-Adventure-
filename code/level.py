...
from database import save_player_progress


class Level:
    def __init__(self, tmx_map, level_frames, audio_files, data, switch_stage, player_id):
    ...
    def level_completed(self):
        return self.player.hitbox_rect.colliderect(self.level_finish_rect)

    def check_level_completion(self):
        # Check if the level is completed
        if self.level_completed:
            next_level = self.data.current_level + 1
            if next_level > self.data.unlocked_level:
                self.data.unlocked_level = next_level
                # Save progress before switching stage
                save_player_progress(self.player_id, next_level)
                # print({next_level}+"has been saved")
            self.switch_stage('overworld', unlock=next_level)
